...
public class RawRenderedImage implements RenderedImage {
	...
	public RawRenderedImage(String filename, long fileOffset, int bitsAllocated, int bitsStored, int width, int height, int tileWidth, int tileHeight) {

		this.filename=filename;
		initialize(fileOffset,bitsAllocated,bitsStored,width,height,tileWidth,tileHeight);
	}

	// =======================================================================

	private void initialize(long fileOffset, int bitsAllocated, int bitsStored, int width, int height, int tileWidth, int tileHeight) {
		...
		maxPixelValue=(int)Math.pow(2,minBitsAllocated)-1;
	...
	public Raster getTile(int tileX, int tileY) {
		...
		if ((tileX>maxTileX)||(tileY>maxTileY)||(tileX<0)||(tileY<0)) {
			String errorString=new String("Error in RawRenderedImage - Illegal tile call ("+tileX+","+tileY+")");
	...
	private Raster getTile(int bitsAllocated, int tileX, int tileY) throws IOException {
								...
								for (int y=0; y<tileHeight; y++) {
									for (int x=0; x<tileWidth; x++) {
										for (int z=0; z<numBands; z++) {
											blockArray[(tileWidth*y)+x+z]=byteArray[(width*y)+xCoord+x];
										}
									}
								}
								byteArray=blockArray;
							}
							db=new DataBufferByte(byteArray,size,0);
							break;
		...
		case 4: int[] intArray=new int[size*numBands];
