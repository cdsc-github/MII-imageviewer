...
public class AnalyzeRenderedImage implements RenderedImage {
	...
	public AnalyzeRenderedImage(String filename, long fileOffset, int bitsAllocated, int fileType, int maxPixelValue, 
	...
	private void initialize(long fileOffset, int bitsAllocated, int fileType, int maxPixelValue, int width, int height, int tileWidth, int tileHeight, ByteOrder bo) {
		...
		switch (fileType) {
			           ...
			           case AnalyzeHeader.GRAY8: sampleModel=RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_BYTE,width,height,numBands); break;
		     case AnalyzeHeader.GRAY16_SIGNED: sampleModel=RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_USHORT,width,height,numBands); break;
		   case AnalyzeHeader.GRAY16_UNSIGNED: sampleModel=RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_USHORT,width,height,numBands); break;
		        case AnalyzeHeader.GRAY32_INT:
          case AnalyzeHeader.GRAY32_FLOAT: sampleModel=RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_INT,width,height,numBands); break;
				      ...
				      try {
							...
							for (int i=0; i<size; i+=numBands) {
								for (int j=0; j<numBands; j++) {
									short s=(bo==ByteOrder.LITTLE_ENDIAN) ? 
										(short)((char)((bArray[(i*2)+1+j] & 0xffff) << 8) | (char)(bArray[(i*2)+j] & 0x00ff)) :
										(short)((char)((bArray[(i*2)+j] & 0xffff) << 8) | (char)(bArray[(i*2)+1+j] & 0x00ff));

									tileArray[i+j]=(short)(((s>=maxPixelValue) ? 0 : ((s<0) ? 0 : s))); 
								}
							}
							...
							db=(sampleModel.getDataType()==DataBuffer.TYPE_SHORT) ? new DataBufferShort(tileArray,size) : new DataBufferUShort(tileArray,size);
							break;
