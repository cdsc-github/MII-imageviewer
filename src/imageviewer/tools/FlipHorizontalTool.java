...
import imageviewer.ui.swing.undo.FlipHorizontalEdit;

// =======================================================================

public class FlipHorizontalTool extends ImagingTool implements Tool {
	...
	public void execute(MouseEvent e) {
		...
		if (e.getSource() instanceof ImagePanel) {
			...
			boolean flipState=((Boolean)rp.getProperty(RenderingProperties.HORIZONTAL_FLIP)).booleanValue();
			rp.setProperties(new String[] {RenderingProperties.HORIZONTAL_FLIP},new Object[] {new Boolean(!flipState)});
			FlipHorizontalEdit fve=new FlipHorizontalEdit(ip,flipState);
			ApplicationContext.postEdit(fve);
			ip.fireGroupPropertyChange(new String[] {RenderingProperties.HORIZONTAL_FLIP},new Object[] {new Boolean(!flipState)});
			ip.repaint();
		}
	}
}
